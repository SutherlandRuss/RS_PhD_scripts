...
#scores.file<- "pancan12_cleaned.maf"
...
colonClinical.path <- "/Users/rds/Dropbox/PhD/tumour_classifier_data/SynapsePanCancer/PanCan12/clinical/COAD"
...
rectumClinical.path <- "/Users/rds/Dropbox/PhD/tumour_classifier_data/SynapsePanCancer/PanCan12/clinical/READ"
...
#only of use If I am using the TCGA files. The PanCancer data does not require these two lines
...
summaries<- list(classPerGene=summaryStats.mutationsClassPerGene, typePerGene=summaryStats.mutationsTypePerGene, classPerIndiv=summaryStats.mutationsClassPerIndiv, typePerIndiv=summaryStats.mutationsTypePerIndiv, IndelsPerGene=summaryStats.IndelsPerGene, mutationsSNVPerGene=summaryStats.mutationsSNVPerGene,
                 IndelsPerIndiv=summaryStats.IndelsPerIndiv, mutationsSNVPerIndiv=summaryStats.mutationsSNVPerIndiv, allMutationsPerIndiv=summaryStats.AllMutationsPerIndiv)
summaries<- lapply(summaries, as.data.frame)# convert to dataframes so they can be written to excel multiple worksheets.

install.packages("WriteXLS")
library(WriteXLS)
#setwd("C:/Users/rsutherland/Dropbox/PhD/tumour_classifier_data/analysis/Pan_cancer12/coadRead/Descriptive_analysis")
write.table(summaries,"PanCan_descriptive_stats.txt", append=TRUE, sep="\t", eol="\n", na="NA")
WriteXLS("summaries","PanCan_descriptive_stats.xls", row.names=TRUE)
...
metadata2<- cbind.data.frame(metadata2, seqTech=seqTech[,1], sequenceCenter=sequenceCenter[,1], stringsAsFactors=FALSE)# adds the cancerType and seqTech metadata to the metadata object
...
PanCanstrat<-stratification
save("PanCanstrat", file="/Users/rds/Dropbox/PhD/TCGA_pipeline/MDS_data/PanCanStrat.RData")

# Save the PanCan ISOMDS object
PanCan_ISOMDS<-u.mds
save("PanCan_ISOMDS", file="/Users/rds/Dropbox/PhD/TCGA_pipeline/MDS_data/PanCan_ISOMDS.RData")

#save the mutationMatrixLogical2 (for PCA)

PanCan_mutationMatrixLogical2<-mutationMatrixLogical2
save("PanCan_mutationMatrixLogical2", file="/Users/rds/Dropbox/PhD/TCGA_pipeline/MDS_data/PanCan_mutationMatrixLogical2.RData")
...
PanCan_prcomp_mutTable<-prcomp(t(mutationTable), scale=TRUE)

#correlation of PC1 with metadata
PanCanPC1corSilent<-cor.test(PanCan_prcomp$x[,1], silentMutationsPerIndiv)# correlation with mutation rate
PanCanPC1corSeqTech<-cor.test(PanCan_prcomp$x[,1], PanCanstrat$sequenceCenter[,1])# correlation with sequence center
PanCanPC1corAge<-cor.test(PanCan_prcomp$x[,1], PanCanstrat$age_at_initial_pathologic_diagnosis[,2])# correlation with age
PanCanPC1corGender<-cor.test(PanCan_prcomp$x[,1], PanCanstrat$gender[,1])# correlation with mutation gender
PanCanPC1corCancerType<-cor.test(PanCan_prcomp$x[,1], PanCanstrat$tumourType[,1])# correlation with mutation gender
...
setwd("/Users/rds/Dropbox/PhD/tumour_classifier_data/analysis/Pan_cancer12/coadRead/MDS")
