...
network.path  <- "/Users/rds/Dropbox/PhD/PINA/"
...
scores.path <- "/Users/rds/Dropbox/PhD/tumour_classifier_data/colorectal_somatic_mutations/combined"
scores.file <- "colorectalcancer.maf"



#scores.path <- "/Users/rds/Dropbox/PhD/tumour_classifier_data/colorectal_somatic_mutations/TCGA_nature_somatic_muts"
#scores.file <- "TCGA_nature_colorectal_muts.txt"
...
colonClinical.path <- "/Users/rds/Dropbox/PhD/tumour_classifier_data/colorectal_somatic_mutations/coloncancer/Clinical_17_12_12/Biotab/"
...
rectumClinical.path <- "/Users/rds/Dropbox/PhD/tumour_classifier_data/rectum_adenocarcinoma/Clinical_18_02_2013/Biotab/"
...
AllMutationsClassPerIndiv<-table(scores$Variant_Classification, scores$sampleID)
silentMutationsPerIndiv<- AllMutationsClassPerIndiv[7,]# the silent mutation rate for each sample
...
AllMutationsClassPerIndiv<-table(scores$Variant_Classification, scores$sampleID)
silentMutationsPerIndiv<- AllMutationsClassPerIndiv[9,]# the silent mutation rate for each sample
...
IndelsPerIndiv<-sapply(seq(1:length(intersectSamples)), function(x) sum(mutationsClassPerIndiv[c(1,2),x]))# the columns chosen change depending on which dataset you use, pre-may 2013 or post may 2013
...
mutationsSNVPerIndiv<-sapply(seq(1:length(intersectSamples)), function(x) sum(mutationsClassPerIndiv[3,x]))# number of SNVs per Indiv
...
IndelsPerGene<-sapply(seq(1:dim(mutationsClassPerGene)[2]), function(x) sum(mutationsClassPerGene[c(1,2),x]))# the columns chosen change depending on which dataset you use, pre-may 2013 or post may 2013
names(IndelsPerGene)<-colnames(mutationsTypePerGene)
mutationsSNVPerGene<-sapply(seq(1:dim(mutationsClassPerGene)[2]), function(x) sum(mutationsClassPerGene[3,x]))# number of SNVs per Indiv
...
summaryStats.IndelsPerGene<-c(summary(IndelsPerGene, digits=10),Mean.sd=sd(IndelsPerGene), Mean.stderr= std(IndelsPerGene), Mode=my_mode(IndelsPerGene),Modal.Value=names(my_mode(IndelsPerGene)))
summaryStats.IndelsPerIndiv<-c(summary(IndelsPerIndiv, digits=10),Mean.sd= sd(IndelsPerIndiv), Mean.stderr= std(IndelsPerIndiv), Mode=my_mode(IndelsPerIndiv),Modal.Value=names(my_mode(IndelsPerIndiv)))
summaryStats.mutationsSNVPerGene<-c(summary(mutationsSNVPerGene, digits=10),Mean.sd= sd(mutationsSNVPerGene), Mean.stderr= std(mutationsSNVPerGene), Mode=my_mode(mutationsSNVPerGene),Modal.Value=names(my_mode(mutationsSNVPerGene)))
summaryStats.mutationsSNVPerIndiv<-c(summary(mutationsSNVPerIndiv, digits=10),Mean.sd= sd(mutationsSNVPerIndiv), Mean.stderr= std(mutationsSNVPerIndiv), Mode=my_mode(mutationsSNVPerIndiv),Modal.Value=names(my_mode(mutationsSNVPerIndiv)))
...
summaryStats.mutationsClassPerIndiv<- rbind(summaryStats.mutationsClassPerIndiv, Mean.sd=sapply(seq(1:dim(mutationsClassPerIndiv)[1]), function(x) sd(mutationsClassPerIndiv[x,])),
                                            Mean.stderr=sapply(seq(1:dim(mutationsClassPerIndiv)[1]), function(x) std(mutationsClassPerIndiv[x,])), Mode=sapply(seq(1:dim(mutationsClassPerIndiv)[1]), function(x) my_mode(mutationsClassPerIndiv[x,])), Modal.Value=sapply(seq(1:dim(mutationsClassPerIndiv)[1]), function(x) names(my_mode(mutationsClassPerIndiv[x,]))))
...
summaryStats.mutationsTypePerIndiv<- rbind(summaryStats.mutationsTypePerIndiv, Mean.sd=sapply(seq(1:dim(mutationsTypePerIndiv)[1]), function(x) sd(mutationsTypePerIndiv[x,])),
                                           Mean.stderr=sapply(seq(1:dim(mutationsTypePerIndiv)[1]), function(x) std(mutationsTypePerIndiv[x,])),Mode=sapply(seq(1:dim(mutationsTypePerIndiv)[1]), function(x) my_mode(mutationsTypePerIndiv[x,])), Modal.Value=sapply(seq(1:dim(mutationsTypePerIndiv)[1]), function(x) names(my_mode(mutationsTypePerIndiv[x,]))))
...
summaryStats.mutationsClassPerGene<- rbind(summaryStats.mutationsClassPerGene, Mean.sd=sapply(seq(1:dim(mutationsClassPerGene)[1]), function(x) sd(mutationsClassPerGene[x,])),
                                           Mean.stderr=sapply(seq(1:dim(mutationsClassPerGene)[1]), function(x) std(mutationsClassPerGene[x,])), Mode=sapply(seq(1:dim(mutationsClassPerGene)[1]), function(x) my_mode(mutationsClassPerGene[x,])), Modal.Value=sapply(seq(1:dim(mutationsClassPerGene)[1]), function(x) names(my_mode(mutationsClassPerGene[x,]))))
...
summaryStats.mutationsTypePerGene<- rbind(summaryStats.mutationsTypePerGene,Mean.sd=sapply(seq(1:dim(mutationsTypePerGene)[1]), function(x) sd(mutationsTypePerGene[x,])),
                                          Mean.stderr=sapply(seq(1:dim(mutationsTypePerGene)[1]), function(x) std(mutationsTypePerGene[x,])),Mode=sapply(seq(1:dim(mutationsTypePerGene)[1]), function(x) my_mode(mutationsTypePerGene[x,])), Modal.Value=sapply(seq(1:dim(mutationsTypePerGene)[1]), function(x) names(my_mode(mutationsTypePerGene[x,]))))
...
#non-silent mutationsPer Individual
summaryStats.NonSilentMutationsPerIndiv<-c(summary(mutationsPerIndiv, digits=10),Mean.sd=sd(mutationsPerIndiv), Mean.stderr= std(mutationsPerIndiv), Mode=my_mode(mutationsPerIndiv),Modal.Value=names(my_mode(mutationsPerIndiv)))

#All mutations
AllMutationsPerIndiv <-tapply(scores$Hugo_Symbol, scores$sampleID,length)# total number of mutations Per individual
#all mutations per individual
summaryStats.AllMutationsPerIndiv<-c(summary(AllMutationsPerIndiv, digits=10),Mean.sd=sd(AllMutationsPerIndiv), Mean.stderr= std(AllMutationsPerIndiv), Mode=my_mode(AllMutationsPerIndiv),Modal.Value=names(my_mode(AllMutationsPerIndiv)))


#wrtite out the summary stats

##write this table for the pancancer, TCGAa and intersection data for PANCAN and TCGA data
summaries<- list(classPerGene=summaryStats.mutationsClassPerGene, typePerGene=summaryStats.mutationsTypePerGene, classPerIndiv=summaryStats.mutationsClassPerIndiv, typePerIndiv=summaryStats.mutationsTypePerIndiv, IndelsPerGene=summaryStats.IndelsPerGene, mutationsSNVPerGene=summaryStats.mutationsSNVPerGene,
                 IndelsPerIndiv=summaryStats.IndelsPerIndiv, mutationsSNVPerIndiv=summaryStats.mutationsSNVPerIndiv, allMutationsPerIndiv=summaryStats.AllMutationsPerIndiv, nonSilentMutationsPerIndiv=summaryStats.NonSilentMutationsPerIndiv)
summaries<- lapply(summaries, as.data.frame)# convert to dataframes so they can be written to excel multiple worksheets.




#write the TCGA descriptive stats to file
WriteXLS("summaries","TCGA_descriptive_stats.xls", row.names=TRUE)
...
mutationMatrixLogical2<-mutationMatrixLogical
...
plot(stratMeanMutPIPG[[1]], col=alpha(1,0.2), log="y", pch=20)# The beginings of my plot according to sequencer Type
points(stratMeanMutPIPG[[2]],col=alpha(2,0.2), pch=20)
...
# Hamming distance calculation and plot
p<-hamming.distance(t(mutM))
ph<-p/max(p)
pp<-as.dist(ph)
ppp<-cmdscale(pp,k=2,eig=TRUE)
plot(ppp$points[,1],ppp$points[,2], pch=20)
ppp<-isoMDS(pp,k=2, max=1000, tol=1e-5)

#u<- compDiss(t[c(-178,-179,-156,-164),c(-178,-179,-156,-164)],1,mutM[,c(-178,-179,-156,-164)])
u<- compDiss(t,1,mutM)# my made-up distance

u.mds<- isoMDS(u,k=2, max=1000, tol=1e-5)#non-metric mds of my made up distance
v<-cmdscale(u,k=2, eig=TRUE)#classical mds of my made-up distance

jac<-vegdist(t(mutationMatrixLogical2), method="jaccard")#jaccard similarity
#jaccard distance
jacDist<-1-jac
w<-cmdscale(jacDist,k=2,eig=TRUE)

########################################################################################################
######
# Save the stratification object

TCGA_stratification<-stratification
save("TCGA_stratification", file="/Users/rds/Dropbox/PhD/TCGA_pipeline/MDS_data/TCGA_stratification.RData")

# Save the PanCan ISOMDS object
TCGA_ISOMDS<-u.mds
save("TCGA_ISOMDS", file="/Users/rds/Dropbox/PhD/TCGA_pipeline/MDS_data/TCGA_ISOMDS.RData")

#save the mutationMatrixLogical2 (for PCA)

TCGA_mutationMatrixLogical2<-mutationMatrixLogical2
save("TCGA_mutationMatrixLogical2", file="/Users/rds/Dropbox/PhD/TCGA_pipeline/MDS_data/TCGA_mutationMatrixLogical2.RData")
...
TCGAPC1corSilent<-cor.test(TCGA_prcomp$x[,1], silentMutationsPerIndiv)# correlation with mutation rate
TCGAPC1corSeqTech<-cor.test(TCGA_prcomp$x[,1], TCGA_stratification$seqTech[,1])# correlation with sequence center
TCGAPC1corAge<-cor.test(TCGA_prcomp$x[,1], TCGA_stratification$age_at_initial_pathologic_diagnosis[,2])# correlation with age
TCGAPC1corGender<-cor.test(TCGA_prcomp$x[,1], TCGA_stratification$gender[,1])# correlation with mutation gender
TCGAPC1corCancerType<-cor.test(TCGA_prcomp$x[,1], TCGA_stratification$tumourType[,1])# correlation with mutation gender
