...
#
# Set the filepath
dir.create(paste0("/Users/rds/Dropbox/PhD/CUP_classifier/outputs/11_class_balanced/11_class_test_AUG_",overSample, sep=""))
setwd(paste0("/Users/rds/Dropbox/PhD/CUP_classifier/outputs/11_class_balanced/11_class_test_AUG_",overSample, sep=""))
...
#the number of smote samples defined according to the minority class
minclassSmoteN<- floor(minclass23rds*overSample)
...
get_stratified_sample = function (dt, strat_by, sample_intensity=0.1) {
 ...
 #f=factor(dt[,"outcome"])
  ...
  #
  # sample 2/3 thirds of each class. 
  twothirdSample = floor(tmp$x*sample_intensity)
  
  tmp$sz = rep(minclassSmoteN,dim(tmp)[1])
  tmp$sz<-twothirdSample
  tmp$sz[which(twothirdSample>minclassSmoteN)]<- minclassSmoteN#IF two thirds is at least 200 samples then it is further downsampled to 200
  # this means that the cancer types with a training set below 200 can be upsamples using SMOTE after I have put the unused training samples in the test set
...
mutationMatrixLogicalTrainT<- mutationMatrixLogicalTrain



# define a list the length of the number of classes with training set sizes below 200
SMOTED<-list()

for (i in seq(1:length(which(table(mutationMatrixLogicalTrainT$outcome)<minclassSmoteN)))){
  
  # names of the cancer types with fewer than 200 training samples
  trainTypeTable<-table(mutationMatrixLogicalTrainT$outcome)
  
  if(length(which(trainTypeTable<minclassSmoteN))>0){
    # name and index of the cancer type with the minimum training samples in the training set
    minorityClassName<-names(trainTypeTable)[which(trainTypeTable==min(trainTypeTable))]
    
    
    
    
    
    n2SMOTE<-ceiling((minclassSmoteN-min(trainTypeTable))/min(trainTypeTable))*100
    
    
    SMOTED[[i]]<-SMOTE(outcome~., data=mutationMatrixLogicalTrainT, perc.over=n2SMOTE, perc.under=0)
    names(SMOTED)[[i]]<-minorityClassName
    
    SMOTEDnamesNchar<- nchar(rownames(SMOTED[[i]]))
    SMOTEDsampleIndex<- which(SMOTEDnamesNchar<10)
    # rename the smotedsamples so that I do not get duplicate rownames in my final mutation matrix
    rownames(SMOTED[[i]])[SMOTEDsampleIndex]<- paste(rep(minorityClassName,length(SMOTEDsampleIndex)),rownames(SMOTED[[i]])[SMOTEDsampleIndex], sep="")
    
    
    
    # smoted samples need to be reduced so that all classes are exactly balanced
    # the number of samples to reduce the SMOTE samples by must be calculate
    
    nSmotesamplesRemove<-dim(SMOTED[[i]])[1]-minclassSmoteN
    
    # the index of the smoted samples to be removed from SMOTED[i]
    underSampleSmote<- sample(seq(min(SMOTEDsampleIndex),dim(SMOTED[[i]])[1]),nSmotesamplesRemove)
    
    # the redifined SMOTED sample plues original samples for classification
    SMOTED[[i]]<-SMOTED[[i]][-c(underSampleSmote),]
    
    # convert row names in to a colmn innthe data frame
    name_rows(SMOTED[[i]])
    
    mutationMatrixLogicalTrainT<- mutationMatrixLogicalTrainT[-which(mutationMatrixLogicalTrainT$outcome==minorityClassName),]
    
    # then I need to refactor in order to remove th factor level which no longer exists.
    mutationMatrixLogicalTrainT$outcome<- factor(mutationMatrixLogicalTrainT$outcome)
...
mutationMatrixLogicalTrain<-mutationMatrixLogicalTrainT
mutationMatrixLogicalSMOTED<- ldply(SMOTED, data.frame)

SMOTEDrownames<-unlist(lapply(SMOTED, row.names))
rownames(mutationMatrixLogicalSMOTED)<- SMOTEDrownames

#
# remove the id column in the mutationMatrixLogicalSMOTED
mutationMatrixLogicalSMOTED<- mutationMatrixLogicalSMOTED[,-1]
colnames(mutationMatrixLogicalSMOTED)<-colnames(mutationMatrixLogicalTrain)


# stick the two dataframes together
mutationMatrixLogicalTrain<- rbind(mutationMatrixLogicalTrain,mutationMatrixLogicalSMOTED)

# remove mutationMatrixLogicalSMOTED
rm(mutationMatrixLogicalSMOTED)


###############################
###############################
###############################
# finding the real samples that were not in the training set so that they can be assigned to the test set.


####identifying the real samples in mutationMatrixLogicalTrain
# splitting the rownames in to sets of characetrs
SMOTEDsampleIDS<-strsplit(rownames(mutationMatrixLogicalTrain), split="")
# divide the character sets in to the cancer names. Real samples are identified by TCGA at the start of their string
SMOTEDrealsamples<-which(unlist(lapply(SMOTEDsampleIDS, function(x) paste(x[1:4], collapse="")))=="TCGA")
# sample names for the real SMOTED training set samples
SMOTEDrealsamplesNames<-rownames(mutationMatrixLogicalTrain)[SMOTEDrealsamples]
# names of the training set to be sent to the test set
forTestSet<-setdiff(rownames(mutationMatrixLogicalB), SMOTEDrealsamplesNames)

#############
#############
# This section redifines the test set as the real samples that were not used in the training set

#sample IDS that are not in the Train Set.
testSamples<-forTestSet
trainSamples<- rownames(mutationMatrixLogicalTrain) 

#test MutationMatrixLogical samples
mutationMatrixLogicalTest<-mutationMatrixLogicalB[testSamples,]

#### trainOutcome
trainOutcome<-mutationMatrixLogicalTrain$outcome
...
tumourType<-unique(trainOutcome)
...
write.table(picksizeResults, file=paste("tolerance_results_11class.txt", sep=""), quote=FALSE, sep="\t", col.names=TRUE, row.names=FALSE)
...
pdf(paste("size_tolerance_plots_11class.pdf"), 12,10, pointsize=12)
...
pdf(paste("accuracy_hist_distribution_11class.pdf",sep=""),12,10, pointsize=12)
...
sink(paste("Train_11class_all_genes_randomForest_results.txt", sep=""))
...
write.table(trainModel$optVariables,paste("kegg_protein_list_11_class.txt", sep=""), row.names=FALSE, col.names=FALSE )
...
testClasses<-testClasses[,(c(1,match(levels(testOutcome), levels(testClasses[,1]))+1))]#testing
testClasses[,1]<-factor(testClasses[,1], levels(testOutcome))#testing
write.table(testClasses,file="ROC_curve_data.txt", append=FALSE, quote=FALSE, sep=" ", row.names =TRUE, col.names=TRUE)

#re-level the testOutcome vector be the same as the testclasses factor.
#testOutcome<- factor(testOutcome, levels(testClasses[,1]))
...
for(i in seq(1:length(classStatsSubTypes))){
  ...
  print(paste("ROC RESULTS2",classStatsSubTypes[i]))
  print(test)
  sink()
  ...
  if(i==1){
    ...
    png("11_classifier_ROC.png", width=1200, height=1200)
    par(mar=c(5,5,5,25))
    par(xpd=T)
...
legend(0.40,0.45,legendText,title="AUC",col=c(1:11), cex=2.4, lty=1, lwd=8, bty="n")
dev.off()

## save the random forest object

elevenClassRF_all_features2<-trainModel
save(elevenClassRF_all_features2, file="elevenClassRF_all_features.RData")
print(elevenClassRF_all_features2)
